    ...
    steps {
        ...
        */
        ...
        stage('Build docker image') {
            steps {
                sshagent(['ansible-key']) {
                    sh 'ssh -t -t ubuntu@13.41.75.159 -o StrictHostKeyChecking=no "ansible-galaxy collection install community.docker"'
                    sh 'ssh -t -t ubuntu@13.41.75.159 -o StrictHostKeyChecking=no "cd /etc/ansible && ansible-playbook /opt/docker/docker-image.yml"'
                }
            }
        }
        ...
        stage('Deploy to stage') {
            steps {
                sshagent(['ansible-key']) {
                    sh "ssh -t -t ubuntu@13.41.75.159 -o StrictHostKeyChecking=no \"cd /etc/ansible && ansible-playbook /opt/docker/docker-container.yml\""
                }
            }
        }
