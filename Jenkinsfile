    ...
    steps {
        ...
        */
        ...
        stage('Build docker image') {
            steps {
                sshagent(['ansible-key']) {
                    sh 'ssh -t -t ec2-user@13.40.218.170 -o StrictHostKeyChecking=no "ansible-galaxy collection install community.docker"'
                    sh 'ssh -t -t ec2-user@13.40.218.170 -o StrictHostKeyChecking=no "cd /etc/ansible && ansible-playbook /opt/docker/docker-image.yml"'
                }
            }
        }
        ...
        stage('Deploy to stage') {
            steps {
                sshagent(['ansible-key']) {
                    sh "ssh -t -t ec2-user@13.40.218.170 -o StrictHostKeyChecking=no \"cd /etc/ansible && ansible-playbook /opt/docker/docker-container.yml\""
                }
            }
        }
